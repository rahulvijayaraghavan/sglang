...
def _hadamard_transform_kernel(
    ...
    # Iterative FWHT via butterfly passes using XOR partner indexing.
    # At stage k, partner of element i is i ^ (1 << k).
    for k in tl.static_range(LOG2N):
        h = 1 << k
        partner_offs = offs ^ h
        # Store to scratch, reload partner
        tl.store(row_out + offs, x, mask=mask)
        partner = tl.load(row_out + partner_offs, mask=mask, other=0.0)
        # top element (bit k is 0): a + b; bottom (bit k is 1): a - b
        is_top = (offs & h) == 0
...
def _triton_hadamard_transform(x: torch.Tensor, scale: float) -> torch.Tensor:
